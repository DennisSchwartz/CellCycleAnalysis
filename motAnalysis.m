...
condLabels = cellstr(char('Tx+A83', 'Tx'));
...
cellNmbrsA = calcCellNmbrs(treesA, A);
cellNmbrsB = calcCellNmbrs(treesB, B);
...
legend(condLabels(1),condLabels(2),[condLabels(1),' Fit & Mean'],[condLabels(2), ' Fit & Mean'])
...
title(['Cell-cycle time distribution for each generation ', condLabels(1)]);
ylim([10 25]);
...
xlabel('Generation');
ylabel('Cell-cycle times (hours)');
title(['Cell-cycle time distribution for each generation ', condLabels(2)]);
ylim([10 25]);
hold off;
export_fig;


% Build additional plot for cell numbers
...
results.A = A;
results.B = B;
results.overallDistA = overallDistA;
results.overallDistB = overallDistB;
results.perGenDistA = perGenDistA;
results.perGenDistB = perGenDistB;
results.cellNmbrsA = cellNmbrsA;
results.cellNmbrsB = cellNmbrsB;
